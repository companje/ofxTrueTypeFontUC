...
public:
  ...
  ofMesh getCharacterQuad(const unsigned int &charID, float x, float y);
...
//-----------------------------------------------------------
ofMesh ofxTrueTypeFontUC::Impl::getCharacterQuad(const unsigned int &charID, float x, float y) {
  GLfloat x1, y1, x2, y2;
  GLfloat t1, v1, t2, v2;
  t2 = cps[charID].t2;
  v2 = cps[charID].v2;
  ...
  int firstIndex = 0; //stringQuads.getVertices().size();
  ...
  ofMesh mesh;
  mesh.addVertex(ofVec3f(x1, y1));
  mesh.addVertex(ofVec3f(x2, y1));
  mesh.addVertex(ofVec3f(x2, y2));
  mesh.addVertex(ofVec3f(x1, y2));

  mesh.addTexCoord(ofVec2f(t1, v1));
  mesh.addTexCoord(ofVec2f(t2, v1));
  mesh.addTexCoord(ofVec2f(t2, v2));
  mesh.addTexCoord(ofVec2f(t1, v2));
...
//=====================================================================
vector<ofMesh> ofxTrueTypeFontUC::getCharacterQuads(const string &utf8_src, float x, float y) {
  vector<ofMesh> meshes;

  if (!mImpl->bLoadedOk) {
    ofLog(OF_LOG_ERROR,"ofxTrueTypeFontUC::getCharacterQuads - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    return meshes;
  };
  ...
  while (index < len) {
    ...
    if (utf32_src[index] == (unsigned int)'\n') {
      ...
      meshes.push_back(mImpl->getCharacterQuad(charID, X, Y));
...
void ofxTrueTypeFontUC::bind(unsigned int i) {
  mImpl->bind(i);
}

//=====================================================================
vector<unsigned int> ofxTrueTypeFontUC::getCharacterIDs(const string &utf8_src) {
  vector<unsigned int> charIDs;

  if (!mImpl->bLoadedOk) {
    ofLog(OF_LOG_ERROR,"ofxTrueTypeFontUC::getCharacterIDs - Error : font not allocated -- line %d in %s", __LINE__,__FILE__);
    return charIDs;
  };
